...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const exerciseFilter = req.query.exercise as string; // 'squat' | 'pushup' | 'dumbbell_fly' or undefined

  const databaseUrl = process.env.DATABASE_URL || process.env.POSTGRES_URL;
  let finalLeaderboard = [...MOCK_LEADERBOARD];

  // If filtering mock leaderboard
  if (exerciseFilter && exerciseFilter !== 'overall') {
    const targetName = formatExerciseName(exerciseFilter);
    finalLeaderboard = MOCK_LEADERBOARD.filter(
    ...
    try {
      ...
      if (exerciseFilter && exerciseFilter !== 'overall') {
        query = `
          ...
          WHERE ws.exercise_key = $1
          GROUP BY ws.user_id, u.username;
        `;
        values = [exerciseFilter];
      } else {
        query = `
          SELECT 
            ws.user_id,
      ...
      const combined = [...dbEntries, ...finalLeaderboard];
